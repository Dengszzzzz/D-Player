...
/**
 * 丢包 AVPacket 压缩包，需要考虑关键字
 * */
void dropAVPacket(queue<AVPacket *> &q){
    while(!q.empty()){
        AVPacket *avPacket = q.front();
        if (avPacket->flags != AV_PKT_FLAG_KEY){  //非关键字，可以丢弃
...
VideoChannel::VideoChannel(int stream_index, AVCodecContext *codecContext, AVRational time_base, int fps) : BaseChannel(
...
void VideoChannel::video_play() {
    ...
    while (isPlaying){
        ...
        if (time_diff > 0){
            //视频时间 > 音频时间，要等音频，所以控制视频播放慢一点
            if (time_diff > 1){
                // 说明：音频预视频差值很大，TODO 拖动条 特色场景  音频 和 视频 差值很大，我不能睡眠那么久，否则是大Bug
